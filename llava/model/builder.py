...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map='auto',
                          attn_implementation="eager", customized_config=None, overwrite_config=None,
                          **kwargs):
    ...
    if ("llava" in model_name.lower() or is_multimodal) and "merged" not in model_name.lower():
        # Load LLaVA model
        if "lora" in model_name.lower() and model_base is None:
            warnings.warn(
                "There is `lora` in model name but no `model_base` is provided. If you are loading a LoRA model, "
                "please provide the `model_base` argument. Detailed instruction: https://github.com/haotian-liu/LLaVA"
        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            elif "telechat" in model_name.lower():
                print("*"*20)
                print("loading telechat...")
                from llava.model.language_model.llava_telechat import LlavaTelechatConfig

                lora_cfg_pretrained = LlavaTelechatConfig.from_pretrained(model_path, trust_remote_code=True)
                tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False, trust_remote_code=True)
                model = LlavaTelechatForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True,
        ...
        elif model_base is not None:  # this may be mm projector only, loading projector with preset language model
            ...
            if "mixtral" in model_name.lower():
                ...
                tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False, trust_remote_code=True)
                from llava.model.language_model.llava_telechat import LlavaTelechatConfig
                cfg_pretrained = LlavaTelechatConfig.from_pretrained(model_path, trust_remote_code=True)
                print(f"cfg_pretrained: {cfg_pretrained}")
                model = LlavaTelechatForCausalLM.from_pretrained(
                    model_base,
            ...
            if "mixtral" in model_name.lower():
                ...
                model = LlavaMixtralForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True,
                                                                attn_implementation=attn_implementation,
                                                                config=llava_cfg, **kwargs)

            elif "mistral" in model_name.lower() or "zephyr" in model_name.lower():
                tokenizer = AutoTokenizer.from_pretrained(model_path)
                model = LlavaMistralForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True,
                                                                attn_implementation=attn_implementation, **kwargs)
            elif (
                "wizardlm-2" in model_name.lower()
                and "vicuna" in model_name.lower()
                ...
                if "moe" in model_name.lower() or "A14B" in model_name.lower():
                    ...
                    if overwrite_config is not None:
                        ...
                        for k, v in overwrite_config.items():
                            setattr(llava_cfg, k, v)
                        model = LlavaQwenMoeForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True,
                                                                        attn_implementation=attn_implementation,
                                                                        config=llava_cfg, **kwargs)
                    else:
                        model = LlavaQwenMoeForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True,
                ...
                tokenizer = AutoTokenizer.from_pretrained(model_path, use_fast=False)
                cfg_pretrained = AutoConfig.from_pretrained(model_path)
                model = LlavaGemmaForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, config=cfg_pretrained,
                                                              attn_implementation=attn_implementation, **kwargs)
            elif "telechat" in model_name.lower():
                tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False, trust_remote_code=True)
                from llava.model.language_model.llava_telechat import LlavaTelechatConfig
                cfg_pretrained = LlavaTelechatConfig.from_pretrained(model_path)
                model = LlavaTelechatForCausalLM.from_pretrained(
                    model_path,
                    low_cpu_mem_usage=True,
                    config=cfg_pretrained,
        ...
        if "merged" in model_name.lower():
            if "telechat" in model_name.lower():
    ...
    if "llava" in model_name.lower() or is_multimodal:
        ...
        vision_tower = model.get_vision_tower()
            ...
            # print("111:",vision_tower.vision_tower_name)
        #    vision_tower.load_model(device_map=device_map)
        if "merged" not in model_name.lower():
    ...
    if hasattr(model.config, "max_sequence_length"):
        ...
        context_len = 2048
    model.save_pretrained("/data/LLaVA-NeXT/checkpoints/llava-telechat2-7B-VL")
    tokenizer.save_pretrained("/data/LLaVA-NeXT/checkpoints/llava-telechat2-7B-VL")
