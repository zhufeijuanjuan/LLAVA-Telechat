...
class TrainingArguments(transformers.TrainingArguments):
    ...
    save_strategy: str = field(default="steps", metadata={"help": "Strategy to use for saving models."})
    save_steps: int = field(default=1000, metadata={"help": "Number of updates steps between two saves."})
    save_all_weight: bool = field(default=False)
    ...
    for conversation, target in zip(conversations, targets):
        ...
        if cur_len < tokenizer.model_max_length:
            if cur_len != total_len:
                target[:] = IGNORE_INDEX
                print(f"WARNING3: tokenization mismatch: {cur_len} vs. {total_len}." f" (ignored)")
...
def preprocess_telechat(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, max_len=2048,
                        system_message: str = "你是一个AI智能助手。") -> Dict:
    roles = {"human": "<_user>", "gpt": "<_bot>"}
    ...
    # Reset Qwen chat templates so that it won't include system message every time we apply
    chat_template = "{%- for message in messages -%}{%- if message['role'] == '<_user>' -%}{{- '<_user>' + message['content'] +'<_bot>' -}}{%- elif message['role'] == '<_bot>' -%}{{- message['content'] + '<_end>' -}}{%- endif -%}{%- endfor -%}"
    tokenizer.chat_template = chat_template
    ...
    for i, source in enumerate(sources):
        ...
        # New version, use apply chat template
        # Build system message for each sentence
        input_id += tokenizer.apply_chat_template([{"role": "<_system>", "content": system_message}])
        target += [IGNORE_INDEX] * len(input_id)
...
def preprocess(sources: Sequence[str], tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False) -> Dict:
    ...
    if conversation_lib.default_conversation.version == "telechat":
        return preprocess_telechat(sources, tokenizer, has_image=has_image)
    # add end signal and concatenate together
    conversations = []
    for source in sources:
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    if model_args.model_class_name is not None:
        ...
        if "mixtral" in model_args.model_name_or_path.lower():
        ...
        elif "telechat" in model_args.model_name_or_path.lower():
            ...
            for name, param in model.named_parameters():
                print(name)
                if 'word_embedding.weight' in name:
                    param.data._rename(name.replace('word_embedding', 'embed_tokens'))
        else:
